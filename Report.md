...
For this comparison graph, we include all four algorithms evaluating the speedup for a random input type with an array size of 2^16. For a speedup graph, we would initially expect to see the speedup scaling linearly with the number of processors with the speedup being equal (speedup of 2 for 2 processors). However, we notice that the values instead are decreasing as the number of processors increases. Only bitonic sort and radix sort initially increase, while merge sort and sample sort show a consistent decreasing trend. This indicates that the algorithms may face limitations in efficiently utilizing additional processors as with the higher processor counts the speedup is decreasing. We also observe that radix sort seems to be more consistently higher than the other sorting algorithms, indicating that this algorithm may be more efficiently parallelizing than the other algorithms. As such, radix sort more efficiently splits tasks between the processors showing more care to not overload the processors. Sample sort seems to have the lowest speedup usually as with this algorithm there is more communication overhead and likely load imbalances due to the data being split between processors during the bucket exchange and partitioning phase. What we can understand from this graph is that different sorting algorithms have varying levels of efficiency when it comes to parallelization, and they do not all scale in the same way with an increase in the number of processors. The expectation of linear speedup is not met for these algorithms, which indicates that parallel processing introduces complexities and challenges. As such, the graph illustrates that while algorithms like Radix Sort can handle parallelism more effectively up to a certain point, others like Sample Sort and Merge Sort may face significant limitations due to communication costs and computation imbalances. This underscores the importance of understanding each algorithm’s parallel characteristics and choosing the right one based on the available computational resources and the size of the data set.
...
For the larger array size, we notice that the values are now increasing rather than decreasing, highlighting that the algorithms are now scaling and improving performance for larger array sizes. This further highlights that the issue before may have been the fact that with less data there are fewer values for each processor to sort leading to more room for communication overhead to overstep, while causing the processors to not effectively distribute tasks. As such during these runs, one processor could have been overloaded, diminishing the effectiveness of parallelism as the time is more close to that of a sequential algorithm. However, we still do notice that the algorithms do not scale linearly, once again highlighting the fact that the performance of the algorithm is slighted by communication overhead, synchronization costs, and inefficiencies with load distribution. Similar to the smaller array size, radix sort still has a higher speedup than the other algorithms, emphasizing that radix sort may be able to more efficiently parallelize sorting and distribute tasks among processors than the other algorithms. Similarly, we note that for a larger array size the effects of paralleism are more clearly shown as with more data values there will be more equal loads for each processor to sort through. 
